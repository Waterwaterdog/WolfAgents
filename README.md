...
- ✅ 完整的狼人杀游戏流程（夜晚/白天阶段）
- ✅ 9 个角色的独立行为逻辑
- ✅ 双模式前端：`admin` 查看 9 Agent 对局，`user` 进入 8 Agent + 1 名人类玩家模式
- ✅ 智能体之间的自然语言交互
- ✅ 游戏状态管理和胜负判定
- ✅ 详细的游戏日志记录系统
- ✅ 玩家经验更新与保存
...
- ✅ 用户模式公平性约束：右侧公屏隐藏 AI `thought/心声`，夜间提示统一显示在画面下方，避免通过卡片位置泄露角色身份
...
- `admin`：进入管理员视角，保留原有 9 Agent 对局界面，适合演示、观察与导出日志。
- `user`：进入人类参与模式，固定由 1 号作为人类玩家，和 8 个 AI 一起完成整局游戏。
- `user` 模式下为保证公平性，右侧公屏只展示发言、行为和系统事件，不展示任何 AI 心声。
- `user` 模式下夜间角色提示（如狼人夜聊、狼人投票、女巫行动、预言家行动）统一显示在画面下方，不再贴附到某个玩家卡片上。
- 当 1 号玩家拿到狼人身份时，会对狼队友进行明确标记；其他非狼玩家不会因此暴露具体角色身份。
