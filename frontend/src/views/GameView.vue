<template>
  <div class="app" style="background: var(--background-current);">
    <div class="header">
      <Header
...
import { DEFAULT_AGENTS, API_URL, BUBBLE_LIFETIME_MS, TYPING_LIFETIME_MS, ASSETS, WS_URL } from "../config/constants";
...
const setBackground = (phase) => {
  const root = document.documentElement;
  if (!root) return;
  if (phase === "night") {
    root.style.setProperty("--background-current", "var(--background-night)");
  } else {
    root.style.setProperty("--background-current", "var(--background-day)");
  }
};

const buildWsUrl = (base, token) => {
  if (!token) return base;
  const sep = base.includes("?") ? "&" : "?";
  return `${base}${sep}token=${encodeURIComponent(token)}`;
};

const submitTurn = (payload) => {
  if (!payload?.requestId) return;
  const client = clientRef.value;
  if (client) {
    client.send({ type: "player_action", payload });
  }
...
onMounted(() => {
  ...
  if (user.value?.username) {
    agents.value = agents.value.map((a) => {
      if (a.id !== "player_1") return a;
      const avatarUrl = user.value?.avatarUrl;
      return { ...a, name: `${user.value.username}·1号`, avatar: avatarUrl || a.avatar, isHuman: true };
    });
  }
  ...
  const onEvent = (evt) => {
    ...
    if (evt.type === "private_notice") {
      addSystemMessage(String(evt.content || ""));
      return;
    }
  ...
  const wsUrl = buildWsUrl(WS_URL, sessionToken);
  const client = new ReadOnlyClient(onEvent, { wsUrl });
